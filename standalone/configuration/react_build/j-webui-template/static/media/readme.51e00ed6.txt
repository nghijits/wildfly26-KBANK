...
trong đó, 
	...
	2. foTable: config của bảng danh sách flow FO,
	3. saveButton: config button Save
Mẫu stateData của FlowFO:
...
stateData = {
		...
		data: [
			{
				...
				components: {
					//danh sách config component trong tab
					txcode: {
						// mã component
						dataFull: {
							config: {
	...
	foTable: {
		...
		//data hiển thị lên table, có dạng json, key: id của header khai báo ở trên, value: value hiển thị
		data: [
			{
				order: 1,
				txCode: "DPT_CDP",
